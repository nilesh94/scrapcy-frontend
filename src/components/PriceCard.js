...
const PriceCard = ({ item, averagePrice, isUnlocked, onUnlock, className = "" }) => {
  ...
  // --- HELPER: FORMAT SPECS ---
  // Joins Location, Grade, Form intelligently
  const getSpecs = () => {
      const parts = [
          item.location, 
          item.grade, 
          item.form,
          !item.grade && !item.form ? item.type : null // Fallback to 'type' if form/grade missing
    ...
    <div className={`relative bg-white rounded-xl border border-platinum shadow-lg hover:shadow-2xl hover:border-orange transition-all duration-300 overflow-hidden flex flex-col ${className}`}>
      ...
      <div className="p-5 pb-0 flex justify-between items-start">
        <div>
           ...
           <div className="text-xs font-bold text-steel uppercase mt-1 tracking-wider flex flex-wrap items-center gap-1">
             {specs.map((spec, index) => (
                <React.Fragment key={index}>
                    {index > 0 && <span className="text-orange">•</span>}
                    <span>{spec}</span>
                </React.Fragment>
             ))}
           </div>
      ...
      <div className="text-center px-5 pb-5 flex-grow">
        <div className="flex items-baseline justify-center gap-1">
            ...
            {item.unit && <span className="text-xs font-bold text-steel">/{item.unit}</span>}
        </div>
        {averagePrice && (
